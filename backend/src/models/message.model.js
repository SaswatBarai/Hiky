...
messageSchema.statics.markMessagesAsDeliveredOnUserOnline = async function (
  userId
) {
  try {
    ...
    const result = await this.updateMany(
      {
        ...
        senderId: { $ne: userId }, // Don't mark own messages as delivered to self
        isDeleted: false,
        "deliveredTo.userId": { $ne: userId }, // Only update if not already delivered
      },
      {
        $addToSet: {
          deliveredTo: {
...
//If user is online click any room having unread message we will mark all message as read
messageSchema.statics.markRoomMessagesAsReadOnOpen = async function (
  userId,
  roomId
) {
